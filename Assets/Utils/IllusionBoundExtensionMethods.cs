...
namespace VirtualDream.Utils
{
    ...
    public static class SpriteBatchDraw
    {
        //[StructLayout(LayoutKind.Sequential, Pack = 1)]
        public struct CustomVertexInfo4 : IVertexType
        {
            ...
            private static VertexDeclaration _vertexDeclaration = new VertexDeclaration(new VertexElement[12]
                {
                    ...
                    new VertexElement(48, VertexElementFormat.Vector3, VertexElementUsage.Position, 2),
                    new VertexElement(60, VertexElementFormat.Color, VertexElementUsage.Color, 2),
                    new VertexElement(64, VertexElementFormat.Vector2, VertexElementUsage.TextureCoordinate, 2),
                    new VertexElement(72, VertexElementFormat.Vector3, VertexElementUsage.Position, 3),
                    new VertexElement(84, VertexElementFormat.Color, VertexElementUsage.Color, 3),
                    new VertexElement(88, VertexElementFormat.Vector2, VertexElementUsage.TextureCoordinate, 3)
                });//这里问别人吧（
            public CustomVertexInfo4(CustomVertexInfo vertex0, CustomVertexInfo vertex1, CustomVertexInfo vertex2, CustomVertexInfo vertex3)
            {
                Position0 = new Vector3(vertex0.Position, vertex0.TexCoord.Z);
                Color0 = vertex0.Color;
                TextureCoordinate0 = new Vector2(vertex0.TexCoord.X, vertex0.TexCoord.Y);

                Position1 = new Vector3(vertex1.Position, vertex1.TexCoord.Z);
                Color1 = vertex1.Color;
                TextureCoordinate1 = new Vector2(vertex1.TexCoord.X, vertex1.TexCoord.Y);

                Position2 = new Vector3(vertex2.Position, vertex2.TexCoord.Z);
                Color2 = vertex2.Color;
                TextureCoordinate2 = new Vector2(vertex2.TexCoord.X, vertex2.TexCoord.Y);

                Position3 = new Vector3(vertex3.Position, vertex3.TexCoord.Z);
                Color3 = vertex3.Color;
                TextureCoordinate3 = new Vector2(vertex3.TexCoord.X, vertex3.TexCoord.Y);
            }
        ...
        //public static void Draw(this SpriteBatch spriteBatch, Texture2D texture, CustomVertexInfo4 vertexInfo)
        //{
        //    var type = typeof(SpriteBatch);
        //    var state = BindingFlags.NonPublic | BindingFlags.Instance;
        //    if ((SpriteSortMode)type.GetField("sortMode", state).GetValue(spriteBatch) != SpriteSortMode.Deferred) return;//sortMode
        //    spriteBatch.Draw(texture, default, default, default, 0, default, 1, 0, 0);
        //    var index = (int)type.GetField("numSprites", state).GetValue(spriteBatch) - 1;
        //    var array = type.GetField("vertexInfo", state).GetValue(spriteBatch);
        //    var value = array[index];
        //    var _type = value.GetType();
        //    _type.GetField("Position0", state).SetValue(value, vertexInfo.Position0);
        //    _type.GetField("Position1", state).SetValue(value, vertexInfo.Position1);
        //    _type.GetField("Position2", state).SetValue(value, vertexInfo.Position2);
        //    _type.GetField("Position3", state).SetValue(value, vertexInfo.Position3);

        //    _type.GetField("Color0", state).SetValue(value, vertexInfo.Color0);
        //    _type.GetField("Color1", state).SetValue(value, vertexInfo.Color1);
        //    _type.GetField("Color2", state).SetValue(value, vertexInfo.Color2);
        //    _type.GetField("Color3", state).SetValue(value, vertexInfo.Color3);

        //    _type.GetField("TextureCoordinate0", state).SetValue(value, vertexInfo.TextureCoordinate0);
        //    _type.GetField("TextureCoordinate1", state).SetValue(value, vertexInfo.TextureCoordinate1);
        //    _type.GetField("TextureCoordinate2", state).SetValue(value, vertexInfo.TextureCoordinate2);
        //    _type.GetField("TextureCoordinate3", state).SetValue(value, vertexInfo.TextureCoordinate3);
        //}
    }
    public static class VirtualDreamDrawMethods
    {
        ...
        public static void DrawQuadraticLaser_PassHeatMap(this SpriteBatch spriteBatch, Vector2 start, Vector2 unit, int HeatMapIndex = 15, float length = 3200, float width = 512, float shakeRadMax = 0, float light = 4, int styleIndex = 10, bool timeOffset = false, float maxFactor = 0.5f, bool autoAdditive = true, (float x1, float y1, float x2, float y2) texcoord = default)
        {
            Effect effect = IllusionBoundMod.GetEffect("Effects/EightTrigramsFurnaceEffect");
            if (autoAdditive)
            {
            ...
            if (bars1.Count > 2)
            {
                ...
                Main.graphics.GraphicsDevice.Textures[0] = IllusionBoundMod.AniTexes[6];
                Main.graphics.GraphicsDevice.Textures[1] = IllusionBoundMod.AniTexes[styleIndex];
                Main.graphics.GraphicsDevice.Textures[2] = IllusionBoundMod.HeatMap[HeatMapIndex];
                Main.graphics.GraphicsDevice.SamplerStates[0] = SamplerState.PointWrap;
                Main.graphics.GraphicsDevice.SamplerStates[1] = SamplerState.PointWrap;
                Main.graphics.GraphicsDevice.SamplerStates[2] = SamplerState.PointWrap;
                if (timeOffset)
        ...
        public static void DrawQuadraticLaser_PassHeatMap(this SpriteBatch spriteBatch, (Vector2 start, Vector2 unit)[] startAndUnits, int HeatMapIndex = 15, float length = 3200, float width = 512, float shakeRadMax = 0, float light = 4, int styleIndex = 10, bool timeOffset = false, float maxFactor = 0.5f, bool autoAdditive = true)
        {
            Effect effect = IllusionBoundMod.GetEffect("Effects/EightTrigramsFurnaceEffect");
            if (autoAdditive)
            {
