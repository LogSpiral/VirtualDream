...
namespace VirtualDream.Contents.StarBound.NPCs.Bosses.AsraNox
{
    ...
    public class AsraNox : ModNPC
    {
        ...
        public static int solusKatanaFractal;
        ...
        private void SpawnFractal(Vector2 targetPosition, Vector2 offset = default, Vector2? dir = null)
        {
            ...
            for (int num80 = 0; num80 < num79; num80++)
            {
                ...
                var proj = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), position1, vector34, solusKatanaFractal, 45, 4, Main.myPlayer, num83);
                if (proj.ModProjectile is SolusKatanaFractal _solusKatanaFractal)
                {
                    _solusKatanaFractal.drawPlayer = new Player();
                    _solusKatanaFractal.drawPlayer.CopyVisuals(visualPlayer);
                }
            }
        }
    ...
    (NPC.life < NPC.lifeMax * 3 / 10 && state < AsraNoxState.陨日残阳_后撤) ||
    (NPC.life < NPC.lifeMax / 4 && state < AsraNoxState.初源日炎_后撤) ||
    (NPC.life < NPC.lifeMax / 5 && state < AsraNoxState.星恒飞刃_后撤) ||
    (NPC.life < NPC.lifeMax * 3 / 20 && state < AsraNoxState.日曜星流_后撤) ||
    (NPC.life < NPC.lifeMax / 10 && state < AsraNoxState.太阳风暴_后撤) ||
    (NPC.life < NPC.lifeMax / 20 && state < AsraNoxState.破晓之光_后撤);
            //Music = MusicID.Boss2;
            switch (state)
            {
                #region 开始
        ...
        private void _1_1()
        {
            ...
            //    ai4 = targetPlayer.Center.X;
            //    ai5 = targetPlayer.Center.Y;
            //    ai6 = Projectile.NewProjectile(NPC.GetSource_FromAI(), new Vector2(direct == 1 ? -1024 : 1024, 0) + new Vector2(ai4, ai5) + new Vector2(0, 1) * ai2, new Vector2(direct == 1 ? 2048 : -2048, ai3) / 20f, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 1);
        ...
        private void _1_2()
        {
            ...
            if (counter == 0)
            {
                ai4 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 2);
            ...
            if (projectile.ModProjectile is SolusKatanaFractal _skf) _skf.updateOldDataMyself = false;
            for (int n = 59; n > 3; n--)
            {
            ...
            if (counter <= 60)
            {
                ...
                for (int n = 0; n < 4; n++)
                {
                    projectile.oldPos[n] = projectile.oldPos[n] == default ? currentVec : Vector2.Lerp(currentVec, projectile.oldPos[4], n * .25f);//
                    if ((int)ai1 < 720)
        ...
        private void _1_3()
        {
            ...
            if (timer < 540)
            {
                ...
                if (counter % 60 <= 30)
                {
                    ...
                    for (int n = 0; n < 4; n++)
                    {
                        projectile.oldPos[n] = projectile.oldPos[n] == default ? currentVec : Vector2.Lerp(currentVec, projectile.oldPos[4], n * .25f);//currentVec;
            ...
            else
            {
                if (timer == 540)
                {
                    ai5 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 4);
                    ...
                    ai6 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 4);
                    Main.projectile[(int)ai6].extraUpdates = 0;
                    ...
                    ai7 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 4);
                    ...
                    ai8 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 4);
                ...
                var _count = 0;
            _mylabel:
                if (_projectile.ModProjectile is SolusKatanaFractal _skf) _skf.updateOldDataMyself = false;
                for (int n = 59; n > 3; n--)
                {
                ...
                for (int n = 0; n < 4; n++)
                {
                    _projectile.oldPos[n] = _projectile.oldPos[n] == default ? currentVec : Vector2.Lerp(currentVec, _projectile.oldPos[4], n * .25f);//currentVec;
            ...
            //    if (counter == 180)
            //    {
            //        ai5 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 2);
            //        Main.projectile[(int)ai5].extraUpdates = 0;
            //        if (Main.projectile[(int)ai5].ModProjectile is SolusKatanaFractal solusKatanaFractal_0)
            //        {
            //            solusKatanaFractal_0.drawPlayer = new Player();
            //            solusKatanaFractal_0.drawPlayer.CopyVisuals(visualPlayer);
            //        }
            //        ai6 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 2);
            //        Main.projectile[(int)ai6].extraUpdates = 0;
            //        if (Main.projectile[(int)ai6].ModProjectile is SolusKatanaFractal solusKatanaFractal_1)
            //        {
            //            solusKatanaFractal_1.drawPlayer = new Player();
            //            solusKatanaFractal_1.drawPlayer.CopyVisuals(visualPlayer);
            //        }
            //        ai7 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 2);
            //        Main.projectile[(int)ai7].extraUpdates = 0;
            //        if (Main.projectile[(int)ai7].ModProjectile is SolusKatanaFractal solusKatanaFractal_2)
            //        {
            //            solusKatanaFractal_2.drawPlayer = new Player();
            //            solusKatanaFractal_2.drawPlayer.CopyVisuals(visualPlayer);
            //        }
            //        ai8 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 2);
            //        Main.projectile[(int)ai8].extraUpdates = 0;
            //        if (Main.projectile[(int)ai8].ModProjectile is SolusKatanaFractal solusKatanaFractal_3)
            //        {
            //            solusKatanaFractal_3.drawPlayer = new Player();
        ...
        private void _1_5()
        {
            ...
            else if (timer < 630)
            {
                ...
                if (counter == 0)
                {
                    ai2 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 2);
                    Main.projectile[(int)ai2].extraUpdates = 0;

                    ai3 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, default, solusKatanaFractal, 45, 4, Main.myPlayer, 0, 2);
        ...
        private void _2_5()
        {
            ...
            _mylabel:
                ...
                for (int n = 0; n < 4; n++)
                {
                    _projectile.oldPos[n] = _projectile.oldPos[n] == default ? currentVec : Vector2.Lerp(currentVec, _projectile.oldPos[4], n * .25f);//
                    var rotation = -timer + n * .25f;
                    _projectile.oldRot[n] = MathHelper.Pi / 10f * rotation - 3 * (float)Math.Sin(MathHelper.Pi / 30f * rotation) + _count * MathHelper.Pi;
                    //if (_count % 2 == 1) _projectile.oldRot[n] = MathHelper.Pi - _projectile.oldRot[n];
                }
        ...
        private void _4_3()
        {
            ...
            if (projectile.type == solusKatanaFractal)
            {
                for (int n = 59; n > 3; n--)
                {
                    projectile.oldPos[n] = projectile.oldPos[n - 4];
                ...
                if (counter % 60 <= 30)
                {
                    ...
                    if (counter % 60 > 21 && counter % 2 == 0)
                    {
                        ...
                        //    //proj.timeLeft = 31;
                        //}
                        var unit = (targetPlayer.Center - NPC.Center).SafeNormalize(default);
                        var factor = counter % 60 - 21f;
                        factor /= 8f;
                        var currentAngle = factor * factor;
                        if (stager % 2 == 1) currentAngle = 1 - currentAngle;
                        for (int n = 0; n < 4; n++)
                        {
                            for (int k = 0; k < stager / 2 + 1; k++)
                            {
                                if (!Main.rand.NextBool(3))
                                {
                                    var proj = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center, unit.RotatedBy((n + currentAngle - 2) * MathHelper.PiOver4) * 5 * ((k + 1f) / (stager / 2 + 1f)), solusEnergyShard, 45, 4, Main.myPlayer, Main.rand.Next(new int[] { 4, 5, 6, 6 }), 1.05f);
                                    proj.friendly = false;
                                    proj.hostile = true;
                                }
        ...
        private void _4_6()
        {
            ...
            else
            {
                ...
                if (timer % 60 == 30)
                {
                    for (int n = 0; n < 3; n++)
                    {
                        var targetPos = new Vector2(NPC.Center.X - Main.rand.Next(0, Main.rand.Next(0, Main.rand.Next(0, 960))) * AsraNoxSky.windDirection, targetPlayer.Center.Y + 540);
                        var proj1 = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center, (targetPos - NPC.Center) / 15, solusEnergyShard, 45, 4, Main.myPlayer, 7, 1f);
                        proj1.friendly = false;
                        proj1.hostile = true;
                        proj1.timeLeft = 46;
    ...
    public class SolusKatanaFractal : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.localAI[0] >= timeToLate)
            {
                ...
                if ((int)projectile.ai[1] == 0)//(int)projectile.ai[1] != 2 && (int)projectile.ai[1] != 3 && (int)projectile.ai[1] != 4
                {
                    for (int n = 59; n > 0; n--)
                    {
                        projectile.oldPos[n] = projectile.oldPos[n - 1];
