...
{
    public class KluexStaff : GlowItem
    {
    ...
    public class KluexStaffProj : RangedHeldProjectile
    {
        ...
        public override void OnCharging(bool left, bool right)
        {
            if (left)
            {
                if (plasmaBallCount < UpgradeValue(60, 120, 180) && (int)Projectile.ai[0] % UpgradeValue(24, 18, 12) == 0)
                {
                    ...
                    {
                        case 0:
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Main.MouseWorld, default, type, 0, 6, Player.whoAmI, plasmaBallCount);
                            break;
                        case 1:
                            vec = ((float)IllusionBoundModSystem.ModTime2).ToRotationVector2() * 32;
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Main.MouseWorld + vec, new Vector2(0, 0), type, 0, 6, Player.whoAmI, plasmaBallCount);
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Main.MouseWorld - vec, new Vector2(0, 0), type, 0, 6, Player.whoAmI, plasmaBallCount);
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Main.MouseWorld, new Vector2(0, 0), type, 0, 6, Player.whoAmI, plasmaBallCount);
                            break;
                        case 2:
        ...
        public override void OnRelease(bool charged, bool left)
        {
            if (left)
            {
                ...
                if (plasmaBallCount > 0)
                {
                    ...
                    if ((int)Projectile.ai[0] % UpgradeValue(10, 8, 6) == 0)
                    {
                        ...
                        if (count > 0) SoundEngine.PlaySound(Terraria.ID.SoundID.Item68);
                        plasmaBallCount--;
            ...
            else
            {
                ...
                if (Charged)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Main.MouseWorld, default, ModContent.ProjectileType<KluexZone>(), 0, 0, Projectile.owner, UpgradeValue(1, 2, 3));
    ...
    public class KluexPlasmaBall : ModProjectile
    {
        public override void SetStaticDefaults()
        {
    ...
    public class KluexPlasmaCrystal : ModProjectile
    {
        Projectile projectile => Projectile;
    ...
    public class KluexZone : ModProjectile
    {
        public override void SetStaticDefaults()
        {
