...
{
    public abstract class Gauntlets : ModItem
    {
        ...
        public override void HoldItem(Player player)
        {
            if (player.whoAmI == Main.myPlayer)
            {
                ...
                {
                    Projectile.NewProjectile(player.GetSource_ItemUse(item), player.Center, new Vector2(0, -1), projType, player.GetWeaponDamage(item), 0f, player.whoAmI);//player.GetWeaponKnockback(item, item.knockBack)
                }
            }
        }
    }
    public abstract class GauntletsProj : ModProjectile
    {
    ...
    public class GauntletProj : GauntletsProj
    {
        ...
        {
            base.OnHitNPC(target, damage, knockback, crit);
            if (WhenSA) target.velocity.Y -= 16;
        }
