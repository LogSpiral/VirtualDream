...
{
    public class TeslasWrath : GlowItem
    {
    ...
    public class TeslasWrathProj : RangedHeldProjectile
    {
        ...
        {
            if (controlState < 3 && Player.PickAmmo(sourceItem, out int _, out float speed, out int damage, out float knockBack, out int _))
            {
                if (charged || left)
                {
                    SoundEngine.PlaySound(SoundID.Item62);
                    Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), ShootCenter, Projectile.velocity * (32f + speed), ModContent.ProjectileType<TeslasWrathBullet>(), Player.GetWeaponDamage(sourceItem) + damage, knockBack + 4f, Player.whoAmI).extraUpdates *= left ? 1 : 2;
        ...
        public override float Factor
        {
            get
            {
                return MathHelper.Clamp(Projectile.ai[0] / UpgradeValue(24f, 18f), 0, 1);
            }
        }
        ...
        public T UpgradeValue<T>(T normal, T extra, T defaultValue = default)
        {
            ...
            if (type == ModContent.ItemType<TeslasWrath>())
            {
                return normal;
            }

            if (type == ModContent.ItemType<TeslasWrathEX>())
            {
                return extra;
            }
    ...
    public class TeslasWrathBullet : ModProjectile
    {
        public override void SetStaticDefaults()
        {
