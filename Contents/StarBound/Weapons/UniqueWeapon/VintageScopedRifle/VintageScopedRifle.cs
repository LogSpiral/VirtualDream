...
{
    public class VintageScopedRifle : ModItem
    {
    ...
    public class VintageScopedRifleProj : RangedHeldProjectile
    {
        ...
        public override void OnRelease(bool charged, bool left)
        {
            if (controlState < 3 && Player.PickAmmo(sourceItem, out int bulletType, out float speed, out int damage, out float knockBack, out int _))
            {
                if (charged || left)
                {
                    ...
                    var proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), ShootCenter, Projectile.velocity * (32f + speed), left ? bulletType : ModContent.ProjectileType<PiercingBullet>(), Player.GetWeaponDamage(sourceItem) + damage, knockBack + 4f, Player.whoAmI);
        ...
        public T UpgradeValue<T>(T normal, T extra, T defaultValue = default)
        {
            ...
            if (type == ModContent.ItemType<VintageScopedRifle>())
            {
                return normal;
            }

            if (type == ModContent.ItemType<VintageScopedRifleEX>())
            {
                return extra;
            }
